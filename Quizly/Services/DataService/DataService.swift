...
//
//            let sessions = resultEntities.compactMap { result -> GameSession? in
//                guard let config = result.configurator else {
//                    return nil
//                }
//
...
//            }
//
//            return sessions.sorted { $0.result.date > $1.result.date }
//
//        } catch {
//            print("❌ Ошибка при загрузке игровых сессий: \(error)")
//            return []
//        }
//    }
//
...
protocol IDataService {
    // Config
    func saveQuestionConfig(_ config: QuestionConfigModel)
    func saveQuizResult(_ result: QuizResultModel)
    
    func setActiveConfigID(_ id: UUID)
    func getActiveConfigID() -> UUID?
    
    func fetchAllGameSessions() -> [GameSession]
}

final class DataService: IDataService {
    private let configService: IQuestionConfigService
    private let resultService: IQuizResultService

    init(configService: IQuestionConfigService, resultService: IQuizResultService) {
...
extension DataService {
    func setActiveConfigID(_ id: UUID) {
        configService.setActiveConfigID(id)
    }
...
extension DataService {
    func fetchAllGameSessions() -> [GameSession] {
        return resultService.fetchAllSessions()
    }
}
